...
def save_and_report(frame, box, label, conf, frame_id, fps, results_list, saved_ids):
    ...
    track_id = f"{int(box[0]/10)}_{frame_id}"
    ...
    try:
        ...
        while cap.isOpened():
            ...
            for i,box in enumerate(boxes):
                ...
                # -----------------------------
                # CENTER CROP (better color detection)
                # -----------------------------
                ...
                if color:
                    ...
                    if not detect_color(crop,color):
                        continue
