...
def parse_prompt(prompt: str):
    ...
    # 🚀 FIXED: Clothing with EXACT color matching
    m = re.search(r'person\s+(?:wearing|wearing\s+a|in|with)\s+(red|blue|black|white|green|yellow|orange|purple|pink|silver|grey|gray|brown)\s+(shirt|tshirt|t-shirt|jacket|coat|salwar|suit|saree|kurti|pant|jeans|top|dress)', p)
    if m:
        color = m.group(1)
    ...
    # 🚀 FIXED: Clothing WITHOUT specific color (any color)
    m = re.search(r'person\s+(?:wearing|wearing\s+a|with)\s+(shirt|tshirt|t-shirt|jacket|coat|salwar|suit|saree|kurti|pant|jeans|top|dress)', p)
    if m:
        return {
            "mode": "person_clothing_any",
            "clothing_type": m.group(1).replace('-', ''),
            "prompt": prompt
        }
    ...
    # 🚀 NEW: ANY bag without color - CHECK BEFORE person_attribute!
    m = re.search(r'person\s+(?:with|have|having|carrying)\s+(bag|backpack|handbag)', p)
    if m:
        return {
            "mode": "person_bag_any",
            "bag_type": m.group(1),
    ...
    # CHECK bag with color patterns BEFORE person_attribute
    m = re.search(r'person\s+(?:with|have|having|carrying)\s+(\w+)\s+(bag|backpack|handbag)', p)
    if m:
        color = m.group(1)
        bag_type = m.group(2)
    ...
    # 🚀 NEW: Shoes detection (any color)
    m = re.search(r'person\s+(?:with|have|having|wear(?:ing)?)\s+(shoes|shoe|footwear|sneakers?|sandals?|boots?)', p)
    if m:
        return {
            "mode": "person_shoes_any",
            "prompt": prompt
        }
    
    # 🚀 NEW: Shoes detection (with specific color)
    m = re.search(r'person\s+(?:with|have|having|wear(?:ing)?)\s+(\w+)\s+(shoes|shoe|footwear|sneakers?|sandals?|boots?)', p)
    if m:
        color = m.group(1)
        if color in COLOR_RANGES:
            return {
                "mode": "person_shoes_color",
                "color": color,
                "prompt": prompt
            }
    
    # 🚀 NEW: Person WITHOUT helmet (no helmet)
    m = re.search(r'person\s+(?:without|without\s+a|without\s+any|no|not\s+having)\s+helmet', p)
    if m:
        return {
            "mode": "person_without_helmet",
            "prompt": prompt
        }
    ...
    # FORCE color_object mode when color + vehicle both present
...
def run_person_without_helmet_mode(cap, results_list):
    """🪖 Detect: Person WITHOUT helmet (violations)"""
    tracker  = CooldownTracker()
    frame_id = 0

    print(f"  Searching: person without helmet")
    ...
    while cap.isOpened():
        ...
        for pbox in p_boxes:
            ...
            # Check if this person has a helmet
            has_helmet = False
            for hbox in h_boxes:
