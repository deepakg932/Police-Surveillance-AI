...
exports.uploadAndProcess = async (req, res) => {
    try {
        ...
        const detections = rawResults.filter(d => d.confidence >= 0.20);
      console.log(detections,"jjjjjjjjjjjjjrrrrr")

        const finalCounts = {};
        const keywords = userPrompt.toLowerCase().split(/[\s,]+/).filter(w => !["with", "and", "a", "wearing"].includes(w));
...
if (detections.length > 0) {
        ...
        const resultsForFrontend = detections.map(d => {
            const safePath = (d.image_path || "detected_frames/default.jpg").replace(/\\/g, "/");
            return {
                ...d,
