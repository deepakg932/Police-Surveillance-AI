...
const DetectionStats = ({ stats, mode }) => {
  ...
  const getLabelForStat = (key) => {
    const labelMap = {
      ...
      totalCommonPersons: "Common Persons",
  ...
  const getMainStats = () => {
    const mainStatKeys = ["totalDetections"];
    return mainStatKeys
          ...
          {mainStats.map(({ label, value, icon: Icon, color }) => (
            ...
            >
              <div className="flex items-center space-x-3">
                <div
                  className={`p-2 ${colorClasses[color]?.bg || colorClasses.gray.bg} rounded-lg`}
                >
                  <Icon
                    className={`h-5 w-5 ${colorClasses[color]?.text || colorClasses.gray.text}`}
                  />
                </div>
                <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {additionalStats.map(({ label, value, icon: Icon, color }) => (
              ...
              >
                <div className="flex items-center space-x-2">
                  <div
                    className={`p-1.5 ${colorClasses[color]?.bg || colorClasses.gray.bg} rounded`}
                  >
                    <Icon
                      className={`h-4 w-4 ${colorClasses[color]?.text || colorClasses.gray.text}`}
                    />
                  </div>
                  <div>
